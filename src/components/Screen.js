...
import digitalDefencePlot from '../plots/digital_defence_plot.json';
import economicDefencePlot from '../plots/economic_defence_plot.json';
...
function Screen({ currentScreen, endMessage, onUpdateScreen, plotFile }) {
  useEffect(() => {
    if (currentScreen >= 0) {
      ...
      document.body.style.setProperty('background-image', `url("${plots[plotFile][currentScreen].backgroundImgPath}")`, 'important');
      return () => {
  ...
  if (currentScreen === -1) { // failure screen
    return (
      <>
  ...
  } else if (currentScreen === -2) { // victory screen
    return (
      <>
  ...
  return (
    <div style={{
      ...
      {
        choices &&
        <SimpleGrid mt='5' w='100%' columns={{sm: 1, md: Math.min(2, choices.length)}} gap='6'>
          {
